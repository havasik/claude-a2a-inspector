...
All input responses SHALL be sent as standard `send_message` socket events with the ARK `input-response` envelope embedded as a DataPart in the message. The backend SHALL process `parts` from the frontend payload and forward DataParts to the agent via the A2A client. The frontend SHALL include the original `taskId` on the message so the agent can correlate the response with the originating task.

#### Scenario: Input response flows through existing pipeline
- **WHEN** any input response is sent
- **THEN** it is emitted as `emit('send_message', { message: "", id: msgId, contextId: state.contextId, taskId: "<original-task-id>", parts: [{ type: "data", data: { ark: { kind: "input-response", ... } } }] })`
...
#### Scenario: Backend forwards taskId on A2A Message
- **WHEN** the backend receives a `send_message` event with a `taskId` field
- **THEN** the backend sets `task_id` on the A2A `Message` object so the agent can correlate the input-response with the original task
...
- **THEN** the agent receives a message with `task_id` set to the original task and a DataPart containing `{ ark: { kind: "input-response", payload: { type: "multi-select", value: ["config", "data"], requestId: "..." } } }`
